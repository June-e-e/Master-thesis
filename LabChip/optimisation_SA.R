...
freq_df <- read.csv("SA_data.csv")

row <- data.frame(pair = 'baseline', pairwise_distance = 0.24164, dH = 0.62, frxfr = 0)
freq_df  <- rbind(freq_df, row)

## Data cleaning
data <- subset(data, Time >= 20 & Time <= 62.27) # filter by running time 72
data[data < 0] <- 0 # replace negative values with 0

ggplot() +
  geom_line(data = data, aes(x = Time, y = SA, colour = "heteroduplex")) +  
  labs(x = "Migration time",
...
for (b in 1:300) {
  ...
  Heteroduplex_binned$SA[b] <- if (nrow(bin_data) > 0) {
...
## Graph before optimisation
ggplot() +
  geom_line(data = Heteroduplex_binned, aes(x = gdH_mean, y = SA, colour = "heteroduplex")) +  
...
ggplot() +
  geom_bar(data = freq_df, aes(x = pairwise_distance, y = frxfr, colour = "nucleotide"), 
           stat = "identity") +   
  labs(x = "Pairwise distance",
       y = "Heteroduplex frequency",
       color = "Legend") +
  theme(legend.position = "none")


ggplot() +
  geom_line(data = data, aes(x = gdH_SA, y = SA, colour = "heteroduplex")) +  
  labs(x = "Normalised migration time",
...
hma_freq <- Heteroduplex_binned$SA
...
q1 <- ggplot() +
  geom_line(data = Heteroduplex_binned, aes(x = estimated_npd, y = SA, colour = "heteroduplex")) +
  geom_line(data = Heteroduplex_binned, aes(x = estimated_npd, y = estimated_nfreq, colour = "optimised_heteroduplex")) +
  geom_bar(data = Nucleotide_binned, aes(x = pd_mean, y = frxfr, colour = "nucleotide"), stat = "identity") + 
  annotate("text", x = -Inf, y = Inf, 
           label = paste("RMSE =", RMSE1), 
...
data$opti_gdH_SA <- (-0.01248065) + 0.27736854 * data$gdH_SA
data$opti_SA <- data$SA * (0.34416230 * exp(18.80257762 * data$opti_gdH_SA))

ggplot() +
  geom_line(data = data, aes(x = opti_gdH_SA , y = opti_SA, colour = "heteroduplex")) + 
...
RMSE3 <- sqrt(sum((dna_freq - Heteroduplex_binned$estimated_nfreq)^2) + sum((npd - Heteroduplex_binned$estimated_npd)^2)) %>%
  round(., digits = 4)
RMSE3 <- "0.4420"

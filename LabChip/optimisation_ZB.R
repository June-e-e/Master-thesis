...
freq_df <- read.csv("ZB_data.csv")

row <- data.frame(pair = 'baseline', pairwise_distance = 0.2156, dH = 0.55, frxfr = 0)
freq_df  <- rbind(freq_df, row)

## Data cleaning
data <- subset(data, Time >= 20 & Time <= 63.10) # filter by running time 72
data[data < 0] <- 0 # replace negative values with 0

ggplot() +
  geom_line(data = data, aes(x = Time, y = ZB, colour = "heteroduplex")) +  
...
for (b in 1:300) {
  ...
  Heteroduplex_binned$ZB[b] <- if (nrow(bin_data) > 0) {
...
ggplot() +
  geom_line(data = Heteroduplex_binned, aes(x = gdH_mean, y = ZB, colour = "heteroduplex")) + 
...
hma_freq <- Heteroduplex_binned$ZB
...
q2 <- ggplot() +
  geom_line(data = Heteroduplex_binned, aes(x = estimated_npd, y = ZB, colour = "heteroduplex")) +
  geom_line(data = Heteroduplex_binned, aes(x = estimated_npd, y = estimated_nfreq, colour = "optimised_heteroduplex")) +
  geom_bar(data = Nucleotide_binned, aes(x = pd_mean, y = frxfr, colour = "nucleotide"), stat = "identity") + 
  annotate("text", x = -Inf, y = Inf, 
           label = paste("RMSE =", RMSE2), 
...
data$opti_gdH_ZB <- (-0.007776805) + 0.229360702 * data$gdH_ZB
data$opti_ZB <- data$ZB * (4.608453533 * exp(11.271586081 * data$opti_gdH_ZB))

ggplot() +
  geom_line(data = data, aes(x = opti_gdH_ZB , y = opti_ZB, colour = "heteroduplex")) + 
  geom_bar(data = freq_df, aes(x = pairwise_distance, y = frxfr, colour = "nucleotide"), stat = "identity") + 
  geom_text(aes(x = 0, y = 0.035, label = paste("RMSE =", RMSE1)), size = 4, color = "blue", hjust = 0) +
